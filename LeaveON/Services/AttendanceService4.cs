    ...
    public List<TimeData> NewMethod1(DateTime startDate, DateTime endDate,  AspNetUser  aspNetUser )
    {
      ...
      catch (SqlException ex)
      {
        ...
        try
        {
          ...
          try
          {
            UserId = aspNetUser.BioStarEmpNum.Value;
          }
          catch (InvalidOperationException ex)
          {
            Console.WriteLine("BioStarEmpNum is NULL");
            Console.WriteLine($"User: {aspNetUser.UserName}");
            throw;
          }
          ...
          {
            Console.WriteLine($"Attendance calculation failed for user {UserId}");
            Console.WriteLine(ex.Message);
          }
