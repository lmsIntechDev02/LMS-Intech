    ...
    public Task ConnectToDBandFillAttendanceData(DateTime startDate, DateTime endDate)
    {
      ...
      foreach (int Id in userIds)
      {
        ...
        */
        ...
        {
          LstThisMonthsWeekEnds = GetWeekEndList(firstDateTime.Year, firstDateTime.Month, aspNetUser.UserLeavePolicy?.WeeklyOffDays ?? "6,0");
        }
        //else
        //{
        ...
        foreach (int weekEndDay in LstThisMonthsWeekEnds)
        {
          ...
          {
            DateTime weekEndDate = new DateTime(firstDateTime.Year, firstDateTime.Month, weekEndDay);
        ...
        int ThisMonthTotalDays = DateTime.DaysInMonth(firstDateTime.Year, firstDateTime.Month);
        for (int absentDay = 1; absentDay <= ThisMonthTotalDays; absentDay++)
        {
          if (LstTimeData.FirstOrDefault(x => x.Date.Day == absentDay) == null)
          {
            blankDateTime = new DateTime(firstDateTime.Year, firstDateTime.Month, absentDay);

            depName = dbLeaveOn.AspNetUsers.FirstOrDefault(x => x.BioStarEmpNum == UserId).DepartmentName;
            attendance = new TimeData() { EmployeeName = UserName, EmployeeNumber = UserId, TimeZone = countryName, Policy = userLeavePolicyDescription, Department = depName, Date = blankDateTime, Day = blankDateTime.DayOfWeek.ToString(), Status = "Absent", isAbsent = true };
            LstTimeData.Add(attendance);
