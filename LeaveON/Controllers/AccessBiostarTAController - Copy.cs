  ...
  public class AccessBiostarTAController : Controller
  {
    ...
    private Task<List<TimeData>> ConnectToDBandReturnWorkingHours(string ReqMonthYear, List<int> UserIds)
    {
      ...
      foreach (int Id in UserIds)
      {
        ...
        cmd = new SqlCommand("select user_id,devdt,bsevtdt,DEVID,devnm from punchlog where USER_ID ='" + UserId + "' and bsevtdt BETWEEN '" + dateAttr[1] + "-" + dateAttr[0] + "-01' AND '" + dateAttr[1] + "-" + dateAttr[0] + "-28' order by bsevtdt", con);//last good
        ...
        DateTime firstDateTime =(DateTime) dt.Rows[0]["bsevtdt"];//ConvertToCountryTimeZone(dt, 0, timeZone);//(DateTime)dt.Rows[0]["SRVDT"];
        DateTime lastDateTime = (DateTime)dt.Rows[rowsCount-1]["bsevtdt"];//ConvertToCountryTimeZone(dt, rowsCount - 1, timeZone);//(DateTime)dt.Rows[rowsCount - 1]["SRVDT"];
        ...
        for (int j = 0; j <= rowsCount - 1; j++)
        {
          ...
          //}
          firstDateTime = (DateTime)dt.Rows[j]["bsevtdt"];//ConvertToCountryTimeZone(dt, j, timeZone);//(DateTime)dt.Rows[j]["SRVDT"];
          ...
          if ((k <= rowsCount - 1) && LstCardReadersIn.Contains((int)dt.Rows[j]["DEVID"]) && !LstCardReadersIn.Contains((int)dt.Rows[k]["DEVID"]) &&
            Convert.ToDateTime(dt.Rows[j]["bsevtdt"]).Day == firstDay && Convert.ToDateTime(dt.Rows[k]["bsevtdt"]).Day == firstDay)
          {
            if (IsCardIn == false)
            {//get first time in
              firsTimeIn = (DateTime)dt.Rows[j]["bsevtdt"];//ConvertToCountryTimeZone(dt, j, timeZone);//(DateTime)dt.Rows[j]["SRVDT"];
              lastTimeOut = (DateTime)dt.Rows[k]["bsevtdt"];//ConvertToCountryTimeZone(dt, k, timeZone);
              IsCardIn = true;
              //UserId = Convert.ToInt32(dt.Rows[j]["USRID"]);
            }

            timeIn = (DateTime)dt.Rows[j]["bsevtdt"];//ConvertToCountryTimeZone(dt, j, timeZone);//(DateTime)dt.Rows[j]["SRVDT"];
            timeOut = (DateTime)dt.Rows[k]["bsevtdt"];//ConvertToCountryTimeZone(dt, k, timeZone);//(DateTime)dt.Rows[j + 1]["SRVDT"];
            ...
            {//get last time out
              IsCardOut = true;
              lastTimeOut = (DateTime)dt.Rows[k]["bsevtdt"];//ConvertToCountryTimeZone(dt, k, timeZone);//(DateTime)dt.Rows[j]["SRVDT"];
            }
          }
    ...
    private DateTime ConvertToCountryTimeZoneNew(DataTable dt, int rowNo, string timeZone)
    {
      ...
      if (timeZone == "" || timeZone == "Pakistan Standard Time")
      {
        ...
        //ConvertedDateTime = TimeZoneInfo.ConvertTimeFromUtc((DateTime)dt.Rows[rowNo]["bsevtdt"], customeTimeZone);
        //return ConvertedDateTime;
        return (DateTime)dt.Rows[rowNo]["bsevtdt"];
      }
      else
      {
    ...
    //    if (timeZone == "")
    //    {
    //      int unixTimeStamp = (int)dt.Rows[rowNo]["bsevtdt"];
    ...
    public async Task<ActionResult> ConnectToDBandReturnOffHours(string reqDate, string UserId)
    {
      ...
      cmd = new SqlCommand("select * from punchlog" + " where USER_ID ='" + UserId + "' and bsevtdt between @fromDate and @toDate order by bsevtdt", con);
      ...
      DateTime LastDate = ConvertToCountryTimeZoneNew(dt,rowsCount-1,timeZone);//ConvertToCountryTimeZone(dt, rowsCount - 1, timeZone);//(DateTime)dt.Rows[rowsCount - 1]["SRVDT"];
      DateTime thisDateTime = ConvertToCountryTimeZoneNew(dt, 0, timeZone);//ConvertToCountryTimeZone(dt, 0, timeZone);//(DateTime)dt.Rows[0]["SRVDT"];
      ...
      for (int j = 0; j <= rowsCount - 1; j++)
      {
        ...
        /////////////////////////////////LstCardReadersIn.Contains((int)dt.Rows[j]["DEVID"])
//        var a1 = LstCardReadersIn.Contains(Convert.ToInt32(dt.Rows[j]["DEVID"]));
//        var a2 = LstCardReadersIn.Contains((int)dt.Rows[j+1]["DEVID"]);
        ...
        {
          timeOut = ConvertToCountryTimeZoneNew(dt, j, timeZone);//(DateTime)dt.Rows[j]["bsevtdt"];//ConvertToCountryTimeZone(dt, j, timeZone);//(DateTime)dt.Rows[j]["SRVDT"];
          timeIn = ConvertToCountryTimeZoneNew(dt, j + 1, timeZone);//(DateTime)dt.Rows[j + 1]["bsevtdt"];//ConvertToCountryTimeZone(dt, j + 1, timeZone);//(DateTime)dt.Rows[j + 1]["SRVDT"];
    ...
    public async Task<ActionResult> WhoIsIn(string reqDate)
    {
      ...
      cmd = new SqlCommand("SELECT user_id, MIN(bsevtdt) bsevtdt from punchlog" + " where bsevtdt between '@fromDate' and '@toDate' GROUP BY USRID order by bsevtdt asc", con);
      //cmd.Parameters.AddWithValue("@fromDate", from_Date.ToString("yyyy-MM-dd HH:mm:ss.fffffff"));
      //cmd.Parameters.AddWithValue("@toDate", to_Date.ToString("yyyy-MM-dd HH:mm:ss.fffffff"));
      cmd.Parameters.AddWithValue("@fromDate", from_Date);
      cmd.Parameters.AddWithValue("@toDate", to_Date);
      ...
      for (int j = 0; j <= rowsCount - 1; j++)
      {
        ...
        FirsTimeIn = ConvertToCountryTimeZoneNew(dt, j, timeZone);//(DateTime)(dt.Rows[j]["bsevtdt"]); //ConvertToCountryTimeZone(dt, j, timeZone);//(DateTime)(dt.Rows[j]["SRVDT"]);
